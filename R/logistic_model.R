...
#'
# logistic_model_stan <- function(t, sg, og, fg_ant, fg_sd, days, ...) {
#   standata <- list(N = , t= t, sg = sg, og = og, fg_ant = fg_ant, fg_sd = fg_sd, days = days)
...
logistic_model_stan <- function(data, pars = c("t", "sg"), og = NULL, fg_ant, fg_sd, days, ...) {
  ...
  standata <- list(
    N = length(t), t = t, sg = sg, og = og, fg_ant = fg_ant, fg_sd = fg_sd, days = days
  )
